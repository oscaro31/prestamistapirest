...
function pagarCuotas($body) {
    ...
    try {
        foreach ($ids as $idDetalle) {
            ...
            $stmt = $pdo->prepare("UPDATE PrestamoDetalle SET Estado = 'Pagado', FechaPagado = NOW(), MontoPagado = MontoCuota WHERE IdPrestamoDetalle = ? AND IdPrestamo = ? AND Estado = 'Pendiente'");
...
function generarAsientoAutomatico($pdo, $tipoAccion, $idPrestamo, $montoCapital, $montoInteres, $fecha, $idusuario) {
    ...
    $stmtAsiento->execute([$fecha, $concepto, $idusuario, 'prestamos', $idPrestamo]);
